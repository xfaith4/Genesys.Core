    ...
    <GroupBox Grid.Row="0" Header="Authentication">
      <Grid>
        ...
        <ComboBox Grid.Column="1" Name="RegionComboBox" IsEditable="True">
          ...
          <ComboBoxItem Content="cac1.pure.cloud"/>
          <ComboBoxItem Content="euw2.pure.cloud"/>
        </ComboBox>
...
function Build-QueryBody {
    ...
    if (-not [string]::IsNullOrWhiteSpace($mt) -and $mt -ne '(any)') {
        $existing = if ($body.ContainsKey('segmentFilters')) { [System.Collections.Generic.List[object]]$body['segmentFilters'] } else { [System.Collections.Generic.List[object]]::new() }
        $existing.Add(@{
            type       = 'and'
    ...
    if ($convRows.Count -gt 0) {
        $existingConv = if ($body.ContainsKey('conversationFilters')) { [System.Collections.Generic.List[object]]($body['conversationFilters']) } else { [System.Collections.Generic.List[object]]::new() }
    ...
    if ($segRows.Count -gt 0) {
        $existingSeg = if ($body.ContainsKey('segmentFilters')) { [System.Collections.Generic.List[object]]($body['segmentFilters']) } else { [System.Collections.Generic.List[object]]::new() }
...
function Show-Results {
    ...
    $byMedia  = $script:allConversations | Group-Object { ($_.participants | Where-Object { $_.purpose -eq 'agent' } | Select-Object -First 1)?.sessions[0]?.mediaType } |
