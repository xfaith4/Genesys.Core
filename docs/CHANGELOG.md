...
- Added the Division Investigation flagship: `Get-GenesysDivisionInvestigation`
  (`Get-GenesysDivisionInvestigationStepDefinition` internally), implementing
  `combinations.investigationRecipes.division-investigation` from
  `catalog/genesys.catalog.json` over eight composed steps — division,
  queues, agents, grants, agentPerformance, divisionPerformance,
  queuePerformance, qualityScores — against the datasets
  `authorization.get.all.divisions`, `authorization.list.division.queues`,
  ...
  `quality.get.evaluations.query`. Added the `authorization.get.division.grants`
  and `analytics.query.conversation.aggregates.division.performance` dataset
  entries to `catalog/genesys.catalog.json` (the endpoints existed but had no
  `datasets` entry to invoke them by); added `division` to
  `Invoke-Investigation`'s `SubjectType` set and to the `subjectType` enum in
  `catalog/schema/investigation.manifest.schema.json`. Full fixture-driven
  integration test suite under `tests/integration/DivisionInvestigation.Tests.ps1`;
  deterministic sample output generated by
  `scripts/New-DemoDivisionInvestigation.ps1` under
  `samples/demo-division-investigation/`. Documented in
  `docs/INVESTIGATIONS.md` § 4.5, cross-referenced from
  `docs/ENDPOINT_COMBINATIONS.md` § 3.
