...
10. [Customer Journey Enrichment (Digital-to-Voice Context)](#10-customer-journey-enrichment-digital-to-voice-context)
...
| 12 | `conversations.get.conversation.summaries` | `conversationId` | Copilot/Agent Assist AI-generated summary — reason for contact and resolution notes in plain language. This is the single most "informative without being a data dump" step: read it first, then pull steps 1–11 only for the parts that need verification. |
| 13 *(tracked digital contact)* | `externalcontacts.get.contact.journey.sessions` | `externalContactId` (from step 2 participants) | Pre-interaction digital journey: web/app sessions, page views, referrer, device — what the customer was doing before they reached out |
| 14 *(tracked digital contact)* | `externalcontacts.get.contact.journey.segments` | `externalContactId` | Behavioural/marketing segment membership active at contact time |
...
- In one paragraph, what actually happened and how was it resolved? (step 12, AI summary)
- What was the customer doing on the web/app immediately before this contact, if they are a tracked visitor? (steps 13–14)

### Read-First: AI Summary (Step 12)

Before walking the full timeline, pull `conversations.get.conversation.summaries`. It returns a
plain-language summary plus reason-for-contact and resolution fields generated by Copilot/Agent
Assist. For most complaint triage this single call answers "what happened" well enough that steps
3–11 are only needed to *verify* a specific detail (was there really a recording gap, was hold time
really that long) rather than to *discover* the story from raw data. This is the intended reading
order for the "informative without being a data dump" goal stated at the top of this document.
...
### Digital Journey Context (Steps 13–14)

If the conversation object's participant roster (step 2) includes a non-null `externalContactId`,
the customer is a tracked visitor and steps 13–14 add pre-interaction context: what pages they
viewed, what channel/device they used, and what behavioural segment they belong to. This turns
"customer called angry" into "customer viewed the billing FAQ three times, abandoned self-service,
then called" — see [§10 Customer Journey Enrichment](#10-customer-journey-enrichment-digital-to-voice-context)
for the full pattern and its executive-reporting counterpart.
...
## 10. Customer Journey Enrichment (Digital-to-Voice Context)

**Subject:** One `conversationId` (per-conversation drill-in) or an org-wide reporting window (executive funnel)
**Use case:** Genesys Journey Management / Predictive Engagement tracks customer web and app
behaviour before they ever contact the organisation. Correlating that digital journey with the
eventual conversation answers a question neither side can answer alone: *did our self-service
fail this customer, and how much contact volume is driven by which digital experience?*

**Core question:** *What did the customer experience digitally before this contact, and — at scale
— how much of our contact volume originates from a specific digital journey?*

### Per-Conversation Drill-In

| Step | Dataset Key | Join Key | What It Adds |
|------|-------------|----------|--------------|
| 1 | `conversations.get.conversation.object` | seed → `conversationId` | Participant roster; look for a non-null `externalContactId` on the customer participant |
| 2 | `externalcontacts.get.contact.journey.sessions` | `externalContactId` | Web/app sessions: page views, referrer, device, channel, session start/end |
| 3 | `externalcontacts.get.contact.journey.segments` | `externalContactId` | Behavioural/marketing segments the contact belonged to at contact time |

This is the same `externalContactId` join used for BYOI/CRM context in §6 — a single external
contact record is the natural join point for *all* pre- and cross-channel context, not just BYOI
attributes.

### Executive Rollup: Digital Journey → Conversation Funnel

| Dataset Key | Grouping | Metrics |
|-------------|----------|---------|
| `analytics.query.journey.aggregates` | `outcomeId`, `segmentId`, daily/weekly | sessionCount, outcome-triggered counts |
| `analytics.query.conversation.aggregates.digital.channels` | `mediaType`, daily | nOffered/nConnected by channel, correlated to the same window |

```
Conversion to interaction rate% = interactions started from tracked sessions / sessionCount
Segment mix% = share of interaction volume attributable to each behavioural segment
```

**Executive presentation:** a funnel chart (digital session → self-service attempt → live
interaction) plus a segment-mix table. This is the piece that lets a director answer "which digital
experience is generating the most avoidable contact volume" instead of only seeing the contact-centre
side of the story in isolation.

**Licensing note:** requires Genesys Journey Management / Predictive Engagement. If the org does not
track digital journeys, `externalContactId` will be null on nearly all participants and this
combination should be skipped rather than run against empty data — do not surface a journey funnel
tile if `sessionCount` is consistently zero across the reporting window.
