...
## Endpoint Combination Catalog Enrichment Findings

- The catalog has two layers: curated `datasets` (consumed by `Invoke-Dataset -Dataset <key>`, each with `endpoint`/`itemsPath`/`paging`/`retry`/optional `redactionProfile`/`validationStatus`) and raw Swagger-derived `endpoints` (3102 entries). A dataset's `endpoint` field is a key into `endpoints` and can differ from the dataset's own key name (e.g. `analytics.query.conversation.aggregates.division.performance` points at the pre-existing raw key `analytics.division.analysis.conversation.aggregates.by.division.oct.15.dec.8`).
- The `combinations` object (`investigationRecipes`/`executiveReportingPlaybooks`/`voiceEngineerPlaybooks`) is documentation-only — nothing in the PowerShell codebase reads it. It is a separate, lighter-weight concept from `docs/INVESTIGATIONS.md`'s code-backed `Invoke-Investigation` composer (real runtime code in `Genesys.Ops`, with a step-descriptor contract, manifest schema, and Track A/B live-validation gating in its §6).
- Sibling-convention rule confirmed by sampling many dataset entries: a dataset scoped to a single `conversationId` GET always overrides to `retry: "rateLimitAware"`, `paging: "none"`, even when the backing raw endpoint's own `pagingProfile`/`retryProfile` fields say `"default"`.
- ANI/DNIS are not literal field names in the Genesys conversation object — they're derived from `participant.calls[0].self/other.addressNormalized`. This means a redaction profile only needs to avoid stripping `address`-pattern fields to preserve ANI/DNIS; no special-case allowlist rule is needed.
- `Assert-Catalog` (`modules/Genesys.Core/Public/Assert-Catalog.ps1`) calls `Resolve-Catalog` (JSON-schema validation, throws on violation) then additionally requires every dataset to have non-empty `endpoint`/`itemsPath`/`paging.profile`/`retry.profile`, and every raw endpoint (post-resolution, normalized into `.key`/`.itemsPath`/`.paging.profile`/`.retry.profile` shape) to have the same. It does not require Pester and ran successfully in this sandbox after installing PowerShell directly.
- This remote sandbox does not have PowerShell pre-installed, but `packages.microsoft.com` is reachable through the egress proxy, so `wget` of `packages-microsoft-prod.deb` + `apt-get install -y powershell` succeeds. `www.powershellgallery.com`, needed for `Install-Module Pester`, is blocked by an explicit organizational proxy policy (403 on CONNECT, confirmed via the proxy status endpoint as `connect_rejected`/policy denial, not destination bot protection) — Pester-based `tests/unit/*.Tests.ps1` cannot run in this environment.
- Two pre-existing, out-of-scope catalog inconsistencies were found but left unfixed (not touched by this session's edits): the `audit-logs` dataset's paging profile name (`"transactionResults"`) doesn't match any key in `profiles.paging`, and the `users` dataset's paging profile name (`"nextUri"`) doesn't match the actual key `"nextUri_default"`.
