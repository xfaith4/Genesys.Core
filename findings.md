...
## 2026-09-16 Endpoint combination evaluation

- `catalog/genesys.catalog.json` already carries 17 investigation recipes, 15 executive playbooks, and 6 voice-engineer playbooks under `combinations`, covering single-conversation, queue, division, agent, team, real-time, BYOI, embeddable-framework, and journey-attribution scopes — reviewed all of them against the task's ask (single conversation / whole-queue / whole-agent / cross-queue division rollups) before adding anything, to avoid duplicating existing coverage.
- Outbound access to `developer.genesys.cloud` is blocked by this environment's egress proxy (`EGRESS_BLOCKED`), so this session could not re-fetch the documentation URLs live; relied on the catalog's existing endpoint/dataset definitions plus prior sessions' documented findings instead.
- Confirmed a genuine, previously-flagged gap: ROADMAP.md "Following opportunities" #1 ("Repeat-contact, transfer-loop, and failed-handoff analysis") had no catalog recipe, even though `apps/ConversationAnalyzer/modules/App.Database.psm1` (`Import-TransferReport`, `report_transfer_flows`, `report_transfer_chains`) already implements the exact hop-chain/loop derivation locally from conversation segment data. Added `transfer-loop-and-failed-handoff-analysis` (investigation recipe), `transfer-loop-and-escalation-kpis` (executive playbook), and `transfer-chain-and-failed-handoff-forensics` (voice-engineer playbook) to the catalog to document that derivation as a reusable, source-grounded combination — matching the app's existing logic rather than inventing a new one.
- Also confirmed no existing recipe scoped to the intersection of one agent and one queue (agent-investigation and queue-investigation each pull in the subject's other queues/agents). Added `agent-queue-intersection-investigation` for the "this agent mishandled this specific queue" complaint pattern, with a three-way comparison (agent-in-queue vs. agent baseline vs. queue baseline) to attribute an anomaly correctly.
- Verified all four new entries reference only dataset/endpoint keys that already resolve in the catalog (replicated the `Catalog.CombinationReferences.Tests.ps1` check in Python since `pwsh` is unavailable in this container) and that the diff to `genesys.catalog.json` is additive (plus the `generatedAt` timestamp bump) with no reformatting of unrelated content — confirmed by round-tripping the original file through `json.load`/`json.dump(indent=2)` and diffing byte-for-byte against the original before editing.
