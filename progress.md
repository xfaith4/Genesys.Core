...
- Audited `docs/ENDPOINT_COMBINATIONS.md` and `catalog/genesys.catalog.json` against the full Genesys Cloud API surface (from the vendored `GenesysCloudAPIEndpoints.json` swagger cache) looking for high-value investigation combinations not yet catalogued. Found Genesys Cloud **Groups** (`/api/v2/groups`) — an ad hoc, cross-queue, cross-division team/skill collective distinct from Division — entirely absent from the catalog, along with division-side RBAC grant enumeration, voicemail (a "not answered" outcome that isn't abandon), and recording annotations (QM evaluator bookmarks).
- Added 8 curated datasets: `groups.search.groups`, `groups.get.single.group`, `groups.get.group.members`, `authorization.get.division.grants`, `authorization.get.subject.access.scope`, `voicemail.get.user.messages`, `voicemail.get.queue.messages`, `conversations.get.conversation.recording.annotations`.
- Added two `combinations.investigationRecipes` entries — `agent-group-investigation` and `division-access-security-audit` — and documented both in `docs/ENDPOINT_COMBINATIONS.md` as new sections 10 and 11, with a "Division vs Group" clarification added to section 3, extension rows added to the existing Agent/Conversation/Queue investigation extension tables (sections 7–9), and the reference matrix (now section 12) extended with two new columns and the new dataset rows.
- Validated the modified catalog against `catalog/schema/genesys.catalog.schema.json` with Python's `jsonschema` (this environment has no `pwsh`, so `tests/unit/CatalogSchema.Tests.ps1` could not be run directly) and confirmed every new dataset reference in the two new recipes resolves against either `datasets` or `endpoints` in the same file — no dangling references. Also confirmed the markdown reference-matrix table has consistent column counts across all rows.
- Updated `docs/CHANGELOG.md` with the session summary. Follow-up for a session with `pwsh` available: run `tests/unit/CatalogSchema.Tests.ps1` and `tests/unit/CatalogResolution.Tests.ps1` for full confirmation, and consider whether `agent-group-investigation`/`division-access-security-audit` warrant real `Get-Genesys*Investigation` composer functions (per the `INVESTIGATIONS.md` contract) rather than remaining documentation-only recipes.
