...
- Evaluated the catalog's endpoints/datasets against Genesys developer documentation (API Explorer, embeddable-framework condensed-conversation-info, BYOI integration/conv-injection guides, blueprints) to find high-value dataset combinations for single-conversation, queue, division/agent-group, and executive-reporting investigations.
- Added 12 new `catalog/genesys.catalog.json` dataset entries, all marked `validationStatus: unvalidated` per `docs/TEST_EVIDENCE_LEVELS.md`: `conversations.get.call.detail` (call-leg ANI/DNIS forensics anchor), `conversations.get.conversation.participant.wrapup`, `conversations.get.conversation.summaries` (Copilot narrative), `speechandtextanalytics.get.conversation.summaries.detail`, `speechandtextanalytics.get.conversation.categories`, `quality.get.conversation.surveys`, `analytics.query.conversation.details.by.queue`, `routing.get.queue.estimated.wait.time`, `analytics.query.conversation.aggregates.division.performance`, `authorization.get.division.grants`, `workforce.get.agent.management.unit`, `workforce.get.adherence.bulk`.
- Derived each new entry's `paging.profile`/`retry.profile` from sibling-convention sampling rather than copying the raw endpoint defaults verbatim — confirmed conversation-scoped single-GET datasets consistently override to `rateLimitAware`/`none` regardless of what the backing raw endpoint specifies.
- Added a new `voice-engineer-call-detail` redaction profile (removes participant display name and external-contact linkage, deliberately preserves ANI/DNIS since neither is a literal field name in the Genesys response shape — confirmed via independent research that ANI/DNIS are derived from `participant.calls[].self/other.addressNormalized`).
- Fixed 6 stale dataset-key references found while cross-checking the existing `combinations` object's `investigationRecipes`/playbooks against the dataset list.
- Validated the catalog with a custom Python cross-reference (every dataset `endpoint`/`paging.profile`/`retry.profile`/`redactionProfile` resolves; only 2 pre-existing, out-of-scope paging-profile-name mismatches found, both predating this session) and then with the real `Assert-Catalog` cmdlet after installing PowerShell 7.6.2 from `packages.microsoft.com` — passed cleanly with zero exceptions.
- Synced `docs/ENDPOINT_COMBINATIONS.md`: updated the header date, added dataset-step rows/Key Joins/Voice Engineer Notes to Sections 1-3, added a division-rollup row to Section 4 Layer 1, added 12 rows to the Section 10 reference matrix, added two new Metric Glossary entries (EWT seconds, adherence percentage), and added the new datasets as documented (not code-wired) candidate steps in the Section 7-9 "Investigation Extensions" tables with an explicit disclaimer that they are not yet part of the Track A/B `Invoke-Investigation` live-validation gate.
- Deliberately left `docs/INVESTIGATIONS.md` untouched: that doc's §6 ties a flagship investigation's "done" status to every composed dataset having passed live `Invoke-Dataset` acceptance, so adding the new `unvalidated` datasets there as formal `Steps[]` would have implied a gating commitment beyond this session's scope.
- Confirmed `scripts/Invoke-Tests.ps1`/Pester-based unit tests cannot run in this environment: `www.powershellgallery.com` returns a 403 organizational policy denial at the egress proxy (`connect_rejected`, confirmed via the proxy status endpoint), distinct from destination-side bot protection. Per proxy policy, did not retry or attempt to route around it; `Assert-Catalog` (no Pester dependency) was used as the authoritative native validation instead.
