...
- Scheduled catalog-evaluation run: cross-checked `catalog/genesys.catalog.json` against the Genesys Cloud API Explorer and the BYOI/Open Messaging (`conv-injection`), embeddable-framework (condensed conversation info), and blueprints developer docs to find high-value endpoint combinations not yet represented in `combinations`.
- Confirmed the existing `combinations` block (single/agent/queue/division conversation investigations, 8 executive reporting playbooks, 5 voice-engineer playbooks) already covers voice-originated conversations, queues, agents, and cross-queue division rollups thoroughly. The gap found: BYOI/Open Messaging (digital-channel) conversations had raw swagger endpoints in `endpoints` (camelCase operationId keys only, e.g. `getConversationsMessagingIntegrationsOpen`) but no curated dotted-key alias, no dataset entry, and no investigation recipe.
- Added 10 dotted-key endpoint aliases and 8 new datasets for BYOI integration config, Open Messaging identity resolution, per-message delivery/read receipts, conversation threading-window settings, supported-content profiles, and external-contact identity/identifier lookups.
- Added a new `digital-messaging-conversation-investigation` recipe (companion to `single-conversation-investigation` for non-voice conversations) joining conversation base → analytics detail → message delivery receipts → BYOI integration config → identity resolution → external contact/identifiers → sentiment → evaluation → survey, with diagnostic signals and executive metrics. Documented that the Embeddable Framework's client-side "condensed conversation info" object has no server-side REST equivalent; the same facts surface via the existing conversation-base/analytics-detail steps.
- Added a new `open-messaging-integration-health-check` voice/digital-engineer playbook (trunk-and-edge-health-check's BYOI equivalent) for org-wide "a digital channel went dark" triage.
- Added a `conversation-investigation-external-contact` redaction profile so the new external-contact datasets don't leak raw PII (address, DOB, tax identifiers) into investigation artifacts.
- Validation: `python3 -m json.tool` parse, `jsonschema.validate` against `catalog/schema/genesys.catalog.schema.json` (PASS), and a script confirming every new recipe/playbook `dataset` reference resolves to an existing `datasets` or `endpoints` key. `pwsh`/Pester were not available in this remote session, so `Assert-Catalog` and the PowerShell catalog test suite (`tests/unit/CatalogSchema.Tests.ps1`, `CatalogResolution.Tests.ps1`) could not be executed here and should be run in a session with PowerShell before relying on this in production.
