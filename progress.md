...
- Followed up on the 2026-05-10 endpoint-combinations work (`catalog/genesys.catalog.json`'s `combinations` section and `docs/ENDPOINT_COMBINATIONS.md`) by re-verifying every claim against the live Genesys Cloud Platform API swagger spec (`developer.genesys.cloud` itself returned 403 in this sandbox, so the canonical spec was pulled from the `MyPureCloud/platform-client-sdk-java` mirror instead).
- Added the missing `getConversationAssistantCopilotcontext` endpoint (raw and friendly-keyed `conversations.get.conversation.assistant.copilotcontext`) — confirmed real in the live spec and previously absent from the catalog.
- Corrected inaccurate BYOI claims in `docs/ENDPOINT_COMBINATIONS.md`: `externalConversationId` and a `POST /api/v2/conversations/providers/{providerId}/calls` injection endpoint do not exist in the current API surface. Replaced with the two verified mechanisms — voice/CRM tie-back via `externalTag` / `Participant.externalContactId` / `getExternalcontactsContact` / `putExternalcontactsConversation`, and the digital-channel Open Messaging inbound family — and split Section 6 accordingly.
- Documented Mean Opinion Score call-quality signals (`mediaStatsMinConversationMos`, `mediaStatsMinConversationRFactor`, per-participant `mediaEndpointStats[].minMos`) in Section 1 and as a new diagnostic-signal line in the `single-call-forensics` voice-engineer playbook.
- Added a WFM Management Unit cross-reference subsection to Section 3 (Division / Agent Group Investigation), tying `workforce.get.management.units` (carries `ManagementUnit.division`), `workforce.get.management.unit.users`, and `workforce.get.management.unit.adherence` back to the division entry point, and added matching rows to the Section 10 dataset matrix.
- Validation: `jsonschema.validate` against `catalog/schema/genesys.catalog.schema.json` passed (3104 endpoints, 111 datasets); a referential-integrity sweep confirmed every `dataset`/`datasetsInOrder` reference inside `combinations` and every dataset-key/operationId reference in `docs/ENDPOINT_COMBINATIONS.md` resolves to a real catalog entry.
- `pwsh` is not installed in this sandbox, so the repo's PowerShell test suites (`tests/unit/CatalogSchema.Tests.ps1`, `Assert-Catalog`, `scripts/Invoke-Smoke.ps1`) could not be run this session; Python-based schema and referential-integrity checks were used as the verification layer instead.
