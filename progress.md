...
- Evaluated the datasets already in `catalog/genesys.catalog.json` against the endpoint combinations documented under `catalog.combinations` (investigation recipes, voice-engineer playbooks, executive-reporting playbooks) and found 19 dangling dataset-key references that did not resolve to any real `catalog.datasets` entry.
- Reconciled each reference: 6 were stale aliases for datasets that already exist under a different name (10 individual occurrences corrected, e.g. `routing.get.queue.wrapup.codes` → `routing.get.queue.wrapup.codes.by.queue`, `telephony.get.sip.message.for.conversation` → `telephony.get.sip.messages.for.conversation`); 13 were genuine gaps, so new `catalog.datasets` entries were added wrapping existing raw endpoints (e.g. `conversations.get.specific.conversation.details`, `conversations.get.call.detail`, `quality.get.conversation.surveys`, `workforce.get.adherence.bulk`, `authorization.get.division.grants`, `analytics.query.conversation.aggregates.division.performance`).
- Found and fixed two real raw-endpoint bugs while verifying the new entries against Swagger ground truth (`GenesysCloudAPIEndpoints.json`): the division conversation-aggregates endpoint had `itemsPath: "$.conversations"` when its response model only exposes `results` (always returned zero items), and the S&TA conversation-summaries-detail endpoint had `pagingProfile: "pageNumber_default"` when its response model has no pagination metadata at all.
- No `pwsh`/Pester is available in this sandbox, so validation was done with Python: full JSON Schema validation against `catalog/schema/genesys.catalog.schema.json`, a port of `Assert-Catalog.ps1`'s required-field checks (including the `Resolve-CatalogProfile` mode/type fallback logic in `Catalog.ps1`, which a first-pass script missed and which explained an initial false-positive batch of "bad retry/paging profile" findings), and a full cross-reference confirming all 71 unique dataset references inside `combinations` now resolve. All checks passed with 0 errors; `git diff --check` is clean on both touched files.
- Updated `docs/ENDPOINT_COMBINATIONS.md` so the 12 newly visible datasets (12 of the 13 new entries were previously undocumented in prose) appear in the right investigation sections: conversation deep-dive (call detail, participant wrapup, S&TA categories/summaries, Copilot summaries), division investigation (division-performance KPI rollup, division grants), real-time monitoring (queue estimated wait time), executive rollup Layer 4 (conversation transcripts), and the Agent Investigation Extensions table (WFM management unit + adherence). Also extended the dataset combination reference matrix and metric glossary, and bumped the doc's "Last updated" date.
