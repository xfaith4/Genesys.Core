...
| Endpoint combination catalog enrichment | complete | Evaluated catalog/raw endpoints against Genesys developer docs to find high-value combinations for single-conversation, queue, division/group, and executive-reporting investigations. Added 12 new `unvalidated` dataset entries (call-leg ANI/DNIS detail, per-participant wrapup, Copilot/S&TA conversation summaries, S&TA topic categories, direct conversation survey lookup, queue-pre-filtered conversation details, real-time queue EWT, division-level performance rollup, division access grants, WFM management-unit lookup, bulk schedule adherence) plus a new `voice-engineer-call-detail` redaction profile, and fixed 6 stale dataset-key references in existing `combinations` recipes/playbooks. `Assert-Catalog` passed; synced `docs/ENDPOINT_COMBINATIONS.md` (steps, key joins, reference matrix, metric glossary, and Section 7-9 investigation-extension candidates) without touching the Track A/B-gated `docs/INVESTIGATIONS.md` contract. |
...
| `scripts/Invoke-Tests.ps1 -InstallDependencies` failed to install Pester; `www.powershellgallery.com` returned 403 on CONNECT | Endpoint combination catalog validation | Confirmed via the proxy status endpoint this is an organizational policy denial (`connect_rejected`), not a destination-side block; per `/root/.ccr/README.md` did not retry or route around it. Installed PowerShell 7.6.2 directly from `packages.microsoft.com` (which is allowed) and used `Assert-Catalog` (no Pester dependency) for native JSON-schema validation instead, which passed cleanly. |
