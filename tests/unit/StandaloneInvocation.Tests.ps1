...
'@
    ...
    It 'reports planned output path and does not create directories when run in WhatIf mode' {
        $output = & pwsh -NoProfile -File ./modules/Genesys.Core/Public/Invoke-Dataset.ps1 -Dataset audit-logs -OutputRoot $script:testOutputRoot -WhatIf 2>&1
    ...
    It 'accepts script-level BaseUri and Headers parameters' {
        $script = @"
. ./modules/Genesys.Core/Public/Invoke-Dataset.ps1
Invoke-Dataset -Dataset 'audit-logs' -BaseUri 'https://api.test.local' -Headers @{ Authorization = 'Bearer test-value' } -OutputRoot '$($script:testOutputRoot)' -WhatIf
"@
        $output = & pwsh -NoProfile -Command $script 2>&1
        $LASTEXITCODE | Should -Be 0
